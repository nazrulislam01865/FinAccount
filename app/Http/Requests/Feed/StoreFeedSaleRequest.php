        ...
        $validator->after(function ($validator): void {
            ...
            foreach ((array) $this->input('payments', []) as $index => $payment) {
                ...
                if ((filled($accountId) || filled($reference)) && blank($amount)) {
                    $validator->errors()->add("payments.$index.amount", 'Enter the amount received through this account.');
                }
            ...
            if (count($accountIds) !== count(array_unique($accountIds))) {
                $validator->errors()->add('payments', 'Use each receive account only once. Combine amounts received through the same account.');
            }
        });
    }
