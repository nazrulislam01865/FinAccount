    ...
    <div class="hg-report-page">
        <div class="hg-page-header hg-report-page-header">
            <div>
                ...
                <p>Settle customer receivables and supplier payables using your existing accounting rules.</p>
            </div>
        </div>
        ...
        <section class="hg-card">
            ...
            <p class="hg-report-note">Receivable collection must debit the selected money account and credit party receivable. Payable payment must debit party payable and credit the selected money account.</p>
                            ...
                            @foreach($report['rows'] as $row)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @elseif($settlementHeads->isEmpty() || $moneyAccounts->isEmpty())
                                            <span class="hg-muted">Create active money account and settlement transaction head first.</span>
                                        @else
                                            <form method="POST" action="{{ route('reports.due-management.settle') }}" class="hg-due-settle-form">
                                                ...
                                                <div class="hg-due-settle-grid">
                                                    ...
                                                    <label>
                                                        <span>Transaction Head</span>
                                                        <select name="transaction_head_id" required>
                                                            <option value="">Select rule</option>
                                                            @foreach($settlementHeads as $head)
                                                                <option value="{{ $head->id }}" @selected((int) old('transaction_head_id') === (int) $head->id)>
                                                ...
                                                <button type="submit" class="hg-btn hg-btn-primary hg-due-settle-button">Post Settlement</button>
                                            </form>
                                        @endif
                                    </td>
                                </tr>
