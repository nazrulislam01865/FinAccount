...
@section('title', 'Accounting Rules Setup | Accounting System')

@push('styles')
<style>
    .accounting-rules-page .table-wrap { overflow-x: auto; }
    .accounting-rules-page .panel-head { align-items: flex-start; gap: 12px; }
    .accounting-rules-page .panel-head h3 { margin-bottom: 4px; }
    .ledger-side-note { display: block; margin-top: 4px; color: var(--muted); font-size: 12px; line-height: 1.45; }
    .accounting-rules-page .actions { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; padding-top: 14px; border-top: 1px solid var(--line); }
    .accounting-rules-page .actions .btn-primary { grid-column: span 2; }
    ...
    @media (max-width: 880px) { .accounting-rules-page .actions { grid-template-columns: 1fr; } .accounting-rules-page .actions .btn-primary { grid-column: span 1; } }
</style>
@endpush
...
<div class="page-title">
    <div>
        <span class="page-label">Accounting Rules Setup</span>
        <h2>Accounting Rules Setup</h2>
...
<div class="layout accounting-rules-page">
    <div class="left-stack">
        <div class="card toolbar five">
            ...
            <button class="btn-primary" type="button" id="addRuleBtn">+ Add Accounting Rule</button>
        </div>
                    ...
                    <tbody id="ruleTable">
                        @forelse($rules as $rule)
                            <tr
                                ...
                                data-update-url="{{ url('/api/accounting-rules-setup/' . $rule->id) }}"
                            >
                                ...
                                <td>
                                    <div class="action-cell">
                                        ...
                                        <form method="POST" data-delete-form action="{{ url('/setup/accounting-rules-setup/' . $rule->id) }}" onsubmit="return confirm('Delete this accounting rule?')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr data-empty="true">
                                <td colspan="9" style="text-align:center;padding:24px;color:var(--muted)">No accounting rules found. Add your first accounting rule.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="table-info-grid ledger-info-grid">
            <div class="card helper-card">
                <h3>How Accounting Rules Work</h3>
                <p>Every Transaction Head + Settlement Type resolves to one Debit account and one Credit account.</p>
                <div class="flow">
    ...
    <aside class="right-stack">
        <div class="card form-panel ledger-form-card">
            <div class="panel-head">
                <div>
                    <h3>Create / Edit Accounting Rule</h3>
            ...
            <form class="form-grid" id="ruleForm" data-frontend-form data-accounting-rule-form data-action="{{ url('/api/accounting-rules-setup') }}" data-store-url="{{ url('/api/accounting-rules-setup') }}" data-success="Accounting rule saved successfully.">
                @csrf
                ...
                <div class="mapping-preview" id="mappingPreview">
                    <strong>Accounting rule preview</strong>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updatePreview() {
        ...
        if (!head.value || !settlement.value || !dr.id || !cr.id) {
            mappingPreview.innerHTML = '<strong>Accounting rule preview</strong>Select Transaction Head, Settlement Type, Debit Account, and Credit Account to preview the accounting effect.';
            return;
        }
