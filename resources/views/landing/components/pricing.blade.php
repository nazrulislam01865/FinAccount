    ...
    <div class="packages">
      @foreach(data_get($landing, 'packages', []) as $package)
        <div class="package {{ data_get($package, 'popular') ? 'popular' : '' }}">
          ...
          <a href="{{ $landingHref(data_get($package, 'button.href', '#contact'), data_get($package, 'button.label')) }}" class="{{ $buttonClass(data_get($package, 'button.style')) }}" data-bn="{{ $txt(data_get($package, 'button.label'), 'bn') }}" data-en="{{ $txt(data_get($package, 'button.label'), 'en') }}">{{ $txt(data_get($package, 'button.label'), $defaultLang) }}</a>
        </div>
      @endforeach
    </div>
      ...
      @foreach(data_get($landing, 'pricing_notes', []) as $note)
        <div class="simple-card">
          ...
          @if(data_get($note, 'button.label'))
            <a href="{{ $landingHref(data_get($note, 'button.href', '#contact'), data_get($note, 'button.label')) }}" class="btn btn-dark" style="margin-top:18px" data-bn="{{ $txt(data_get($note, 'button.label'), 'bn') }}" data-en="{{ $txt(data_get($note, 'button.label'), 'en') }}">{{ $txt(data_get($note, 'button.label'), $defaultLang) }}</a>
          @endif
        </div>
      @endforeach
    </div>
