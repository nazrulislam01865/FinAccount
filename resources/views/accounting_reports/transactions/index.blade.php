...
<div class="txn-list-page">
    ...
    <form method="GET" action="{{ route('accounting-reports.transactions.index') }}" class="card filters" id="transactionFilterForm">
        ...
        <div><label>From Date</label><input id="fromDate" name="from_date" type="date" value="{{ $filters['from_date'] ?? '' }}"></div>
        <div><label>To Date</label><input id="toDate" name="to_date" type="date" value="{{ $filters['to_date'] ?? '' }}"></div>
    ...
    <div class="card table-card">
        <div class="table-head"><div><h3>Transactions</h3><span id="resultText">Showing transaction records from posted voucher data</span></div><button class="btn-ghost" id="bulkBtn" type="button">Bulk Action</button></div>
...
(function () {
    ...
    ['fromDate', 'toDate', 'natureFilter', 'statusFilter'].forEach((id) => {
