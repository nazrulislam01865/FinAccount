...
<div class="financial-report-page">
    <x-report.page-header title="Supplier Payable" subtitle="Supplier-wise payable opening, movement, and closing balance from party-control voucher details.">
        <x-slot:actions>
            <a class="button btn-outline" href="{{ route('accounting-reports.supplier-payables.export', request()->query()) }}">⇩ Export CSV</a>
    ...
    <x-report.table-card title="Payable Ledger" :subtitle="$formatDate($report['from_date']) . ' to ' . $formatDate($report['to_date'])" footer-left="Supplier payable comes from party-control credit minus debit movements." footer-right="Header amount is not used.">
        <div class="table-wrap"><table class="financial-table"><thead><tr><th>Supplier Code</th><th>Supplier</th><th>Ledger</th><th class="amount">Opening</th><th class="amount">Debit</th><th class="amount">Credit</th><th class="amount">Closing</th></tr></thead><tbody>
            ...
            @empty
                <tr data-empty="true"><td colspan="7" class="empty-state">No supplier payable balance found.</td></tr>
            @endforelse
