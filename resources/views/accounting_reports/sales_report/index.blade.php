...
<div class="financial-report-page">
    <x-report.page-header title="Sales Report" subtitle="Sales/income movements from posted journal lines.">
        <x-slot:actions>
            <a class="button btn-outline" href="{{ route('accounting-reports.sales-report.export', array_merge(request()->query(), ['format' => 'xlsx'])) }}">⇩ Export XLSX</a>
            <a class="button btn-outline" href="{{ route('accounting-reports.sales-report.export', array_merge(request()->query(), ['format' => 'pdf'])) }}">⇩ Export PDF</a>
            <button class="btn-ghost" type="button" onclick="window.print()">Print</button>
            <a class="button btn-primary" href="{{ route('accounting-reports.sales-report.index', request()->query()) }}">↻ Refresh</a>
        </x-slot:actions>
    </x-report.page-header>

    <div class="report-summary-grid report-summary-grid-six">
        <x-report.stat-card label="Total Sales" :value="$money($report['total_amount'])" tone="success" />
        <x-report.stat-card label="Entries" :value="$report['entry_count']" tone="muted" />
        <x-report.stat-card label="Sales Ledgers" :value="$report['by_account']->count()" tone="muted" />
    ...
    <form method="GET" action="{{ route('accounting-reports.sales-report.index') }}" class="card report-toolbar report-toolbar-seven accounting-filter-sequence">
        ...
        <div><label>Sales Ledger</label><select name="account_id"><option value="">All Sales Ledgers</option>@foreach($report['accounts'] as $account)<option value="{{ $account->id }}" @selected((string)($filters['account_id'] ?? '') === (string)$account->id)>{{ $account->account_code }} - {{ $account->account_name }}</option>@endforeach</select></div>
        ...
        <x-report.filter-actions :reset-route="route('accounting-reports.sales-report.index')" />
    </form>

    <x-report.table-card title="Sales Detail" :subtitle="$formatDate($report['from_date']) . ' to ' . $formatDate($report['to_date'])" footer-left="Sales report includes Income account lines only." footer-right="Amount = credit minus debit.">
        <div class="table-wrap"><table class="financial-table"><thead><tr><th>Date</th><th>Voucher</th><th>Head</th><th>Party</th><th>Ledger</th><th>Reference</th><th class="amount">Debit</th><th class="amount">Credit</th><th class="amount">Sales Amount</th></tr></thead><tbody>
            ...
            @endforelse
            <tr class="grand-row"><td colspan="8">Total Sales</td><td class="amount">{{ $money($report['total_amount']) }}</td></tr>
        </tbody></table></div>
    </x-report.table-card>
</div>
@endsection
