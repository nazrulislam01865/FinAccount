...
@section('title', 'Manual Journal Entry | Accounting System')
...
<div class="page-header">
    ...
    <a href="{{ route('transactions.create') }}" class="button btn-secondary">Back to Transaction Entry</a>
</div>

<div class="summary-grid" style="grid-template-columns: repeat(auto-fit, minmax(220px, 1fr));">
    <div class="card prototype-stat"><span>Current Financial Year</span><strong>{{ $currentFinancialYear?->display_name ?? 'Not configured' }}</strong><small>Posting date must be open/unlocked</small></div>
    <div class="card prototype-stat"><span>Posting Ledgers</span><strong>{{ $ledgers->count() }}</strong><small>Only active Level 4 ledgers are shown</small></div>
    <div class="card prototype-stat"><span>Recent Manual JVs</span><strong>{{ $recentJournals->count() }}</strong><small>Latest journal vouchers</small></div>
</div>
...
<div class="content-grid setup-grid" style="grid-template-columns:minmax(0, 1.35fr) minmax(320px, .65fr);gap:18px;align-items:start;">
    <form class="card form-card" id="manualJournalForm" data-frontend-form data-action="{{ route('api.manual-journals.store') }}">
        @csrf
        ...
        <div class="form-grid two-cols">
            <div>
                ...
                <input type="date" name="journal_date" value="{{ $defaultJournalDate }}" required>
            </div>
            <div>
            ...
            <table class="data-table" id="journalLinesTable">
                ...
                <tbody id="journalLinesBody">
                    @for($i = 0; $i < 2; $i++)
                        <tr data-line-row>
                            <td>
                                <select name="lines[{{ $i }}][ledger_id]" data-ledger-select required>
                                    <option value="">Select Level 4 ledger</option>
                                    @foreach($ledgers as $ledger)
                                        <option value="{{ $ledger->id }}" data-party-control="{{ $ledger->is_party_control || $ledger->ledger_type === 'Party Control' ? 1 : 0 }}">
                            ...
                            <td>
                                <select name="lines[{{ $i }}][party_id]" data-party-select>
                            ...
                            <td><input type="number" min="0" step="0.01" name="lines[{{ $i }}][debit_amount]" value="0.00" data-debit></td>
                            <td><input type="number" min="0" step="0.01" name="lines[{{ $i }}][credit_amount]" value="0.00" data-credit></td>
                            <td><input type="text" name="lines[{{ $i }}][line_narration]" placeholder="Optional"></td>
        ...
        <div class="mini-list">
            @forelse($recentJournals as $journal)
                <div class="mini-list-item">
                    ...
                    <span>{{ $journal->journal_date?->format('d M Y') }} · {{ $journal->status }} · BDT {{ $money($journal->total_debit) }}</span>
                </div>
            @empty
